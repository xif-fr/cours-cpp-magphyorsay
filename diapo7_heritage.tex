...
\begin{itemize}
\item L'héritage permet de faire intéragir les classes entre elles par
le biais de \structure{relations dites hiérarchiques}
\end{itemize}
...
\begin{itemize}
%\item Une classe \texttt{B} qui hérite d'une classe \texttt{A} a accès aux membres publics et protégés
 % de \texttt{A}
\item Une classe \texttt{B} hérite d'une classe \texttt{A} : les méthodes et fonctions
amies de \texttt{B} ont accès aux membres publics et protégés de \texttt{A}
\end{itemize}
...
\item[{Héritage}] relation de type \structure{\textbf{est un (verbe être)}}; un \texttt{Rectangle} est un
